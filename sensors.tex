...
\noindent with $i$ and $j$ varying belonging to $I = \left[0,3 \right] $ localizing the 16 blocks within the grid. The angles $\theta$ that belong to $\Theta$ are the eight possible equidistant values between $0$ and $315$.  This vector is normalized, clamped to $0.2$, and re-normalized again.   Details of the method can be found on \citep{Ramele2016,Lowe2004}.  It was implemented using the VLFeat  \citep{Vedaldi2010} public Computer Vision libraries.
...
\item Experiment 3 - Component Amplitude Noise: the amplitude of the main P3b component of the ERP template is randomly altered.  This component is defined to be located from the sample point $37$ up to $249$ which is around $840$ ms.  This component multiplied by a gain factor is subtracted from the original template.  This gain factor between $0$ and $1$ is drawn from a uniform distribution $U(0,1)$.  Additionally the subtracted curve is multiplied by a Gaussian window with a support of the same length~\citep{Harris1978}.  This avoids adding any discontinuity into the artificial generated signal.
\end{itemize}
...
Regarding results produced for the public and real dataset of P300 ERP from the Berlin BCI Competition II (2003), the obtained performance is above theoretical chance level, and for some algorithms close to the usable threshold of $70\%$.  It is also similar to the performance obtained for the Experiment 3, which represents coincidentally a more realistic scenario.  It is important to remark that the algorithms presented here analyze the single-channel waveform structure of the signal~\citep{Gribonval2008}.  
