...
\item Experiment 2 - Latency Noise:  Instead of superimposing the P300 ERP over the EEG trace at the exact locations where stimulus onsets are situated, an artificial latency lag is added.  This value is obtained from a uniform distribution ranging from $0$ to $40\%$ of the $1s$ of the window size~\citep{DaPelo2018}.
...
All these experiments are executed using 3-fold cross validation procedure that preserves the structure of the letter identification trials. Spelling letters are scrambled while the order and group of each intensification sequence is preserved.
