...
\begin{equation}
\varepsilon =  \left\lVert   x(n) - \sum_{i=1}^{m} w_i g_{yi}(n)   \right\rVert
\label{eq:mperror}
\end{equation}
...
\begin{equation}
g_{yk}(n) = \arg \max_{g_{yi}} \left\lVert < \tilde{x_{k}}(n), g_{yi}(n) > \right\rVert 
\label{eq:mp3}
\end{equation}
...
\begin{equation}
w_{k} =  \frac{< \tilde{x_{k}}(n), g_{yk}(n) >}{  | g_{yk} | }
\label{eq:mp4}
\end{equation}
...
\begin{equation}
\tilde{x_{k+1}}(n)=  \tilde{x_k}(n) - w_{k} g_{yk} (n), k \leftarrow k + 1
\label{eq:mp5}
\end{equation}

Two variants of this algorithm are evaluated. In \textit{MP 1} the dictionary is directly constructed with the normalized templates from Fig \ref{fig:dictionaryfig}.  In \textit{MP 2} the coefficients of  Daubechies least-asymetric wavelet with 2 vanishing moments atoms are used to construct the dictionary.  For the first version, the matching against the template is evaluated according to Equation \ref{eq:mperror} directly, whereas for the latter each feature is crafted by decomposing the signal in its coefficients and building, an eventually sparse, vector with them.
...
Bond and Pompe Permutation Entropy has been extensively used in EEG processing, with applications on Anesthesia, Sleep Stage evaluation and significantly for Epilepsy pre-ictal detection \citep{Bandt2002}.  This method generates a code based on the orderly arrangement of sequential samples, and then derives a metric which is based on the number of times each sequence is found on the signal synthesized as a numeric value called entropy~\citep{Nicolaou2010}. A signal on a window of length $W$ is represented by 
...
The first step of the algorithm is the plot generation based on single-channel EEG segments $ x(n) $.
...
Once the plot is generated, the plot's center is used to localize the SIFT patch. This region of the image, where the signal's most important salient waveform should be located, is divided in a grid of $4 x 4$ block and the bidimensional gradient vectors are calculated on each one of them.  At the same time, for each block within the patch, a histogram of the gradient orientations, 8 circular orientations, are calculated.  This histogram is concatenated for all the 16 blocks and a feature is thus formed:
...
\noindent with $i$ and $j$ varying between $0$ and $3$, localizing the 16 blocks within the grid. The angles $\theta$ are the eight possible values between $0$ and $315$.  This vector is normalized, clapped to $0.2$, and re-normalized again.   Details of the method can be found on \citep{Ramele2016,Lowe2004}.  It was implemented using the VLFeat  \citep{Vedaldi2010} public Computer Vision libraries.
...
\item Latency Noise: Gaussian noise is added to the ERP template latency, i.e. the temporal location where it is injected into the EEG stream.
\item Component Amplitude Noise: the amplitude of the main P3b  components of the ERP template is altered based on uniform distribution.
\end{itemize}

%\textbf{Borrar o hacer: Finally results for the same methods while offline processing a public dataset of real P300 patterns are also shown.}
...
Fig. \ref{fig:gains} shows a $5s$ sample of the EEG trace.  Channel $S$ represents the twelve different stimulus markers (columns or rows) while channel $L$ represent the label (hit vs nohit).  Subfigure (a) shows the signals before the ERP template is superimposed while subfigure (b) shows the same signals with the superimposed ERP template.  At first sight differences are really hard to spot visually.  Subfigures (c) and (d) show only one second of the same segment only channel Cz and Label.  The superimposed ERP can be divised around $31.5$s where in (d) the peak is bigger.  Fig. \ref{fig:gaincheck} shows the obtained ensemble average ERPs as result of superimposing the template signal into EEG stream, time-locked to the stimulus onset.   These 12 point-to-point averaged segments correspond to the first trial of the EEG stream.
...
\subfigure[The same signal segment with the superimposed template.]{\includegraphics[width=.45\linewidth]{images/singlegain.eps}}
\subfigure[EEG sample of Cz channel of the original EEG trace.]{\includegraphics[width=.45\linewidth]{images/nogainzoomhit.eps}}
...
\caption{Eight-channel EEG signal superimposed with the ERP Template.}
\label{fig:gains}
\end{figure}
...
\caption{Point-to-point averaged signals for the first trial.  The ERP was superimposed on classes 3 and 9.}
\label{fig:gaincheck}
\end{figure}
...
\caption{Speller performance obtained for each method while latencies were artificially added to each single-intensification segment.  The achieved performance is significantly reduced for all methods.}
...
The classification method Support Vector Machine SVM is added for comparison using a feature $f$ constructed by normalizing the signal on each channel~\citep{Krusienski2006}.  
