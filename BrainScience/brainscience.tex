...
The objective of the following experiments is to assess the performance of the algorithms that aim to recognize the shape of the P300 waveform, obtained after averaging signal segments.  This performance is evaluated by processing a pseudo-real dataset with two modalities where subtle alterations on the latency and amplitude of the P300 component are simulated in a controlled environment.

The problem with any P300 experiment is that users are part of a closed loop with the BCI system and the human is typically considered as a static compliant entity that always performs what the experimenter asks for in a precise and consistent way.  Therefore, P300 experiments are often mined with null-signals.  These are EEG streams which are marked as hit but, because the subject was not particularly focused, or concentrated or so on, the real signal is not generated.   This lack of certainty may be in detriment of analysis of algorithms and can be misleading.   Similar experiments had address this same issue, particularly when benchmarking different algorithms.
...
The experiment conducted by~\citep{Riccio2013} is replicated on 4 healthy subjects, performing a copy-spelling task with feedback.  In this case, to construct the pseudo-real dataset each template must first be obtained from each subject.  The most prototypical P300 waveform template is selected by visual observation.  This template is superimposed to each segment marked as \textit{True} on the EEG stream.  The baseline of each segment, on the other hand, is obtained by picking randomly any \textit{False} segment for the same subject.
